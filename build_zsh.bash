...
if [[ -f "$sh_dir/downloads/zsh-5.7.1.tar" ]]; then
    cp "$sh_dir/downloads/zsh-5.7.1.tar" .
else
    rm -f zsh-5.7.1.tar.xz*
    until wget ftp://ftp.zsh.org/zsh/zsh-5.7.1.tar.xz; do
        rm -f zsh-5.7.1.tar.xz*
    done
    xz -d zsh-5.7.1.tar.xz
fi
tar -xf zsh-5.7.1.tar
mkdir "$destdir/src/zsh-5.7.1/build"
cd "$destdir/src/zsh-5.7.1/build" || exit 1
../configure --prefix="$destdir/zsh-5.7.1"
make
make install
cd ~ || exit 1
if [[ -d "$destdir/zsh-5.7.1" ]]; then
    rm "$destdir/src/zsh"* -rf
    cd "$destdir" || exit 1
    ln -s zsh-5.7.1 zsh
