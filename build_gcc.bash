...
until wget http://mirrors.ustc.edu.cn/gnu/gcc/gcc-8.3.0/gcc-8.3.0.tar.gz; do
    rm -f gcc-8.3.0.tar.gz*
done
tar -xf gcc-8.3.0.tar.gz
cd "$destdir/src/gcc-8.3.0" || exit 1
...
mkdir "$destdir/src/gcc-8.3.0/build"
cd "$destdir/src/gcc-8.3.0/build" || exit 1
../configure --prefix="$destdir/gcc-8.3.0" --disable-multilib
make
make install
cd ~ || exit 1
if [[ -d "$destdir/gcc-8.3.0" ]]; then
    rm "$destdir/src/gcc"* -rf
    cd "$destdir" || exit 1
    ln -s gcc-8.3.0 gcc
