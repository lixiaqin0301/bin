...
if [[ -f "$sh_dir/downloads/gcc-10.2.0.tar.gz" ]]; then
    tar -xf "$sh_dir/downloads/gcc-10.2.0.tar.gz"
else
    rm -f gcc-10.2.0.tar.gz*
    until wget http://mirrors.ustc.edu.cn/gnu/gcc/gcc-10.2.0/gcc-10.2.0.tar.gz; do
        rm -f gcc-10.2.0.tar.gz*
    done
    tar -xf gcc-10.2.0.tar.gz
fi
cd "$destdir/src/gcc-10.2.0" || exit 1
...
mkdir "$destdir/src/gcc-10.2.0/build"
cd "$destdir/src/gcc-10.2.0/build" || exit 1
../configure --prefix="$destdir/gcc-10.2.0" --disable-multilib
make
make install
cd ~ || exit 1
if [[ -d "$destdir/gcc-10.2.0" ]]; then
    rm "$destdir/src/gcc"* -rf
    cd "$destdir" || exit 1
    ln -s gcc-10.2.0 gcc
