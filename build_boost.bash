...
if [[ -f "$sh_dir/downloads/boost_1_69_0.tar.gz" ]]; then
    tar -xf "$sh_dir/downloads/boost_1_69_0.tar.gz"
else
    rm -rf boost_1_69_0.tar.gz*
    until wget https://dl.bintray.com/boostorg/release/1.67.0/source/boost_1_69_0.tar.gz --no-check-certificate; do
        rm -rf boost_1.67.0.tar.gz*
    done
    tar -xf boost_1_69_0.tar.gz
fi
cd "$destdir/src/boost_1_69_0" || exit 1
...
./bootstrap.sh --with-python="$pythonexcuable" --prefix="$destdir/boost_1_69_0"
./b2
./b2 install
cd ~ || exit 1
if [[ -d "$destdir/boost_1_69_0" ]]; then
    rm "$destdir/src/boost"* -rf
    cd "$destdir" || exit 1
