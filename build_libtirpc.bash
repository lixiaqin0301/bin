...
if [[ -f "$sh_dir/downloads/libtirpc-1.2.6.tar.bz2" ]]; then
    cp "$sh_dir/downloads/libtirpc-1.2.6.tar.bz2" .
else
    rm -f libtirpc-1.2.6.tar.bz2*
    until wget https://jaist.dl.sourceforge.net/project/libtirpc/libtirpc/1.2.6/libtirpc-1.2.6.tar.bz2; do
        rm -f libtirpc-1.2.6.tar.bz2*
    done
fi
bzip2 -d libtirpc-1.2.6.tar.bz2
tar -xf libtirpc-1.2.6.tar

cd "$destdir/src/libtirpc-1.2.6" || exit 1
#sed -i "s,1\.15,1.16," configure
#sed -i "s,1\.15,1.16," aclocal.m4
#sed -i 's/des_impl.c/des_impl.c des_soft.c/g' src/Makefile.am
./configure --enable-authdes "--prefix=$destdir/libtirpc-1.2.6"
make
make install

cd ~ || exit 1
if [[ -d "$destdir/libtirpc-1.2.6" ]]; then
    #rm "$destdir/src/libtirpc"* -rf
    cd "$destdir" || exit 1
    ln -s libtirpc-1.2.6 libtirpc
