...
if [[ -f "$sh_dir/downloads/llvm-11.0.0.src.tar" ]]; then
    tar -xf "$sh_dir/downloads/llvm-11.0.0.src.tar"
else
    rm -f llvm-11.0.0.src.tar.xz*
    until wget https://github.com/llvm/llvm-project/releases/download/llvmorg-11.0.0/llvm-11.0.0.src.tar.xz; do
        rm -f llvm-11.0.0.src.tar.xz*
    done
    xz -d llvm-11.0.0.src.tar.xz
    tar -xf llvm-11.0.0.src.tar
    rm llvm-11.0.0.src.tar
fi
mv llvm-11.0.0.src llvm
# Clang
cd "$destdir/src/llvm/tools" || exit 1
if [[ -f "$sh_dir/downloads/clang-11.0.0.src.tar" ]]; then
    tar -xf "$sh_dir/downloads/clang-11.0.0.src.tar"
else
    rm -f clang-11.0.0.src.tar.xz*
    until wget https://github.com/llvm/llvm-project/releases/download/llvmorg-11.0.0/clang-11.0.0.src.tar.xz; do
        rm -f clang-11.0.0.src.tar*
    done
    xz -d clang-11.0.0.src.tar.xz
    tar -xf clang-11.0.0.src.tar
    rm clang-11.0.0.src.tar
fi
...
echo "CPP=$cpppath $cmakepath -DCMAKE_C_COMPILER=$ccpath -DCMAKE_CXX_COMPILER=$cxxpath -DCMAKE_BUILD_TYPE=MinSizeRel -DCMAKE_INSTALL_PREFIX=$destdir/llvm-11.0.0 -DLLVM_TARGETS_TO_BUILD=X86 -DLLVM_BUILD_LLVM_DYLIB=1 -DLLVM_OPTIMIZED_TABLEGEN=1 -DCMAKE_CXX_LINK_FLAGS=$cxxlinkflags .." >> "$destdir/src/install_from_src.log"
CPP="$cpppath" "$cmakepath" -DCMAKE_C_COMPILER="$ccpath" -DCMAKE_CXX_COMPILER="$cxxpath" -DCMAKE_BUILD_TYPE=MinSizeRel -DCMAKE_INSTALL_PREFIX="$destdir/llvm-11.0.0" -DLLVM_TARGETS_TO_BUILD=X86 -DLLVM_BUILD_LLVM_DYLIB=1 -DLLVM_OPTIMIZED_TABLEGEN=1 -DCMAKE_CXX_LINK_FLAGS="$cxxlinkflags" ..
...
cd ~ || exit 1
if [[ -d "$destdir/llvm-11.0.0" ]]; then
    rm "$destdir/src/llvm"* -rf
    rm "$destdir/src/clang"* -rf
    cd "$destdir" || exit 1
    ln -s llvm-11.0.0 llvm
    ln -s llvm-11.0.0 clang
