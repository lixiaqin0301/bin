...
if [[ -f "$sh_dir/downloads/git-2.26.2.tar.gz" ]]; then
    cp "$sh_dir/downloads/git-2.26.2.tar.gz" .
else
    rm -f git-2.26.2.tar.gz*
    until wget https://github.com/git/git/archive/v2.26.2.tar.gz -O git-2.26.2.tar.gz; do
        rm -f git-2.26.2.tar.gz*
    done
fi
tar -xf git-2.26.2.tar.gz
cd "$destdir/src/git-2.26.2" || exit 1
make configure
./configure --prefix=$destdir/git-2.26.2
make all doc
make install install-doc install-html
cd ~ || exit 1
if [[ -d "$destdir/git-2.26.2" ]]; then
    rm "$destdir/src/git"* -rf
    cd "$destdir" || exit 1
    ln -s git-2.26.2 git
