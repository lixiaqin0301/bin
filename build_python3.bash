...
if [[ -f "$sh_dir/downloads/Python-3.7.1.tar" ]]; then
    tar -xf "$sh_dir/downloads/Python-3.7.1.tar"
else
    rm -f Python-3.7.1.tar.xz*
    until wget https://www.python.org/ftp/python/3.7.1/Python-3.7.1.tar.xz; do
        rm -f Python-3.7.1.tar.xz*
    done
    xz -d Python-3.7.1.tar.xz
    tar -xf Python-3.7.1.tar
fi
mkdir "$destdir/src/Python-3.7.1/build"
cd "$destdir/src/Python-3.7.1/build" || exit 1
export LD_RUN_PATH=$destdir/Python3/lib
../configure --prefix=$destdir/Python-3.7.1 --enable-shared
make
make install
cd ~ || exit 1
if [[ -d "$destdir/Python-3.7.1" ]]; then
    rm "$destdir/src/Python-3"* -rf
    cd "$destdir" || exit 1
    ln -s Python-3.7.1 Python3
