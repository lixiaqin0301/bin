...
if [[ -f "$sh_dir/downloads/Python-3.9.0.tar" ]]; then
    tar -xf "$sh_dir/downloads/Python-3.9.0.tar"
else
    rm -f Python-3.9.0.tar.xz*
    until wget https://www.python.org/ftp/python/3.9.0/Python-3.9.0.tar.xz; do
        rm -f Python-3.9.0.tar.xz*
    done
    xz -d Python-3.9.0.tar.xz
    tar -xf Python-3.9.0.tar
fi
mkdir "$destdir/src/Python-3.9.0/build"
cd "$destdir/src/Python-3.9.0/build" || exit 1
export LD_RUN_PATH=$destdir/Python3/lib
../configure --prefix=$destdir/Python-3.9.0 --enable-shared
make
make install
cd ~ || exit 1
if [[ -d "$destdir/Python-3.9.0" ]]; then
    rm "$destdir/src/Python-3"* -rf
    cd "$destdir" || exit 1
    ln -s Python-3.9.0 Python3
