...
if [[ -f "$sh_dir/downloads/Python-3.7.4.tar" ]]; then
    tar -xf "$sh_dir/downloads/Python-3.7.4.tar"
else
    rm -f Python-3.7.4.tar.xz*
    until wget https://www.python.org/ftp/python/3.7.4/Python-3.7.4.tar.xz; do
        rm -f Python-3.7.4.tar.xz*
    done
    xz -d Python-3.7.4.tar.xz
    tar -xf Python-3.7.4.tar
fi
mkdir "$destdir/src/Python-3.7.4/build"
cd "$destdir/src/Python-3.7.4/build" || exit 1
export LD_RUN_PATH=$destdir/Python3/lib
../configure --prefix=$destdir/Python-3.7.4 --enable-shared
make
make install
cd ~ || exit 1
if [[ -d "$destdir/Python-3.7.4" ]]; then
    rm "$destdir/src/Python-3"* -rf
    cd "$destdir" || exit 1
    ln -s Python-3.7.4 Python3
