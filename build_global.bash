...
if [[ -f "$sh_dir/downloads/global-6.6.4.tar.gz" ]]; then
    cp "$sh_dir/downloads/global-6.6.4.tar.gz" .
else
    rm -f global-6.6.4.tar.gz*
    until wget http://mirrors.ustc.edu.cn/gnu/global/global-6.6.4.tar.gz; do
        rm -f global-6.6.4.tar.gz*
    done
fi
tar -xf global-6.6.4.tar.gz
mkdir "$destdir/src/global-6.6.4/build"
cd "$destdir/src/global-6.6.4/build" || exit 1
../configure --prefix=$destdir/global-6.6.4
make
make install
cd ~ || exit 1
if [[ -d "$destdir/global-6.6.4" ]]; then
    rm "$destdir/src/global"* -rf
    cd "$destdir" || exit 1
    ln -s global-6.6.4 global
