...
if [[ -d "$rootdir/Python3/lib/python3.8/config-3.8-x86_64-linux-gnu" ]]; then
    python3_config_dir="$rootdir/Python3/lib/python3.8/config-3.8-x86_64-linux-gnu"
else
...
./configure --prefix="$destdir/vim-8.2" --enable-luainterp=yes --enable-mzschemeinterp --enable-perlinterp=yes --enable-python3interp=yes --enable-tclinterp=yes --enable-rubyinterp=yes --enable-cscope --enable-multibyte --with-features=huge --with-python3-config-dir="${python3_config_dir}"
make
make install
cd ~ || exit 1
