...
if [[ -f "$sh_dir/downloads/cmake-3.13.0-rc3.tar.gz" ]]; then
    cp "$sh_dir/downloads/cmake-3.13.0-rc3.tar.gz" .
else
    rm -f cmake-3.13.0-rc3.tar.gz*
    until wget https://cmake.org/files/v3.13/cmake-3.13.0-rc3.tar.gz --no-check-certificate; do
        rm -f cmake-3.13.0-rc3.tar.gz*
    done
fi
tar -xf cmake-3.13.0-rc3.tar.gz
cd cmake-* || exit 1
mkdir build
cd build || exit 1
...
../configure --prefix="$destdir/cmake-3.13.0-rc3"
gmake
gmake install
cd ~ || exit 1
$dnfyum -y remove emacs emacs-common
if [[ -d "$destdir/cmake-3.13.0-rc3" ]]; then
    rm "$destdir/src/cmake"* -rf
    cd "$destdir" || exit 1
    ln -s cmake-3.13.0-rc3 cmake
